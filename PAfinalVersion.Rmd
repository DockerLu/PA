...
```{r}
dat = dat2
#write.xlsx(dat2,file="C:/Users/HP/Dropbox/Team folder/Tingting Sun/PA/dat2.xlsx")
```

#Task3–Use observations from principal components analysis (PCA) to generate a new feature
```{r}
library(openxlsx)
#dat = read.xlsx("C:/Users/HP/Dropbox/Team folder/Tingting Sun/PA/dat2.xlsx")
vars <- colnames(dat)[1:length(dat)-1]
...
#Task5–Select a distribution and link function
```{r}
 # dat = read.xlsx(dat,file="C:/Users/HP/Dropbox/Team folder/Tingting Sun/PA/dat_afterPCA.xlsx")
...
#GLMgamma <- glm(log(gapdays+1) ~ ., family = Gamma(link = "log"), data = train)
GLMgamma <- glm(gapdays+1 ~ ., family = Gamma(link = "log"), data = train)
...
#Add the interactions here selected in Task 4, trial 1
GLMgamma <- glm(gapdays+1 ~ .+ IO_CODE:PATIENT.SEX.CODE + IO_CODE:Age, family = Gamma(link = "log"), data = train)
#GLMgamma <- glm(gapdays+1 ~ + IO_CODE:PATIENT.SEX.CODE, family = Gamma(link = "log"), data = train)
#GLMgamma <- glm(gapdays+1 ~ + IO_CODE:Age, family = Gamma(link = "log"), data = train)
#summary(GLMgamma)
print("AIC")
AIC(GLMgamma)
predict <- predict(GLMgamma,newdata=test,type="response")
...
#Add the interactions here selected in Task 4, trial 2
#GLMgamma <- glm(gapdays+1 ~ + IO_CODE:PATIENT.SEX.CODE + IO_CODE:Age, family = Gamma(link = "log"), data = train)
GLMgamma <- glm(gapdays+1 ~ + IO_CODE:PATIENT.SEX.CODE, family = Gamma(link = "log"), data = train)
#GLMgamma <- glm(gapdays+1 ~ + IO_CODE:Age, family = Gamma(link = "log"), data = train)
#summary(GLMgamma)
...
#Add the interactions here selected in Task 4, trial 3
#GLMgamma <- glm(gapdays+1 ~ + IO_CODE:PATIENT.SEX.CODE + IO_CODE:Age, family = Gamma(link = "log"), data = train)
GLMgamma <- glm(gapdays+1 ~ + IO_CODE:PATIENT.SEX.CODE, family = Gamma(link = "log"), data = train)
#GLMgamma <- glm(gapdays+1 ~ + IO_CODE:Age, family = Gamma(link = "log"), data = train)
#summary(GLMgamma)
